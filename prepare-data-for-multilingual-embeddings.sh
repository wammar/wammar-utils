...
# use up to 10 million sentences from the wikipedia dump of each language
...
do
  ...
  find extracted-${lang} -name 'wiki_*' -print0 | xargs -0 -I file cat file | egrep -v "^[ ^I]*<" | grep "[^ ]" | ~/wammar-utils/europarl-tools/tokenizer.perl -l ${lang} | head -10000000 > mono.${lang}.cased
